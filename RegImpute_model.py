...
from dupl_matrix import createMatrix, createHashtable, blankPredictor
...
logging.basicConfig(
    filename='/var/log/shiny-server/RegImpute_model.log',
    level=logging.INFO,
...
def imputeMatrix(dataM):
    dataN = np.array(dataM)
    nan=np.nan
    model = Ridge(alpha=0.7, fit_intercept=True, normalize=True)
    imputer = StaticImpute(dataN, 'static', 'regressionImpute', model=model, fillmethod="row_median", max_iter=50,mse=1e-9)
    dataT = imputer.impute()
    return dataT
...
def writeFile(infile, currData, annotData):
    ...
    logger.info('Writing output to %s', fn)
